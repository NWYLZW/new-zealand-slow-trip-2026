        ...
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start" spacing={2} className="rental-booking-header">
          <Box>
            ...
            <Typography color="text.secondary" className="rental-reservation">
              {isEnglish ? "Reservation" : "预订代码"} <strong>{booking.reservation}</strong>
            </Typography>
          </Box>
  ...
  return (
    <Stack spacing={3} className="rental-panel">
      <PanelHero
        ...
        desc={isEnglish
          ? "Keep the reservation codes, airport times, payment card and pickup checks handy for the road trip."
          : "旅行中只需关注订单号、机场取还车时间、现场付款和取车检查。"}
      />

      <Grid container spacing={2}>
        {confirmedBookings.map((booking) => (
          <Grid size={{ xs: 12, xl: 6 }} key={booking.reservation}>
