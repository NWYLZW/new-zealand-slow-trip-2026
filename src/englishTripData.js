import { northDays, southDays } from "./tripData";

const eventTranslations = {
  "深圳机场值机、托运行李": "Check in and drop checked baggage at Shenzhen Airport",
  "MH0523 深圳 → 吉隆坡": "MH0523 Shenzhen → Kuala Lumpur",
  "抵达吉隆坡 T1，转机 2小时15分": "Arrive at Kuala Lumpur T1; 2 hr 15 min connection",
  "MH0133 吉隆坡 T1 → 奥克兰": "MH0133 Kuala Lumpur T1 → Auckland",
  "抵达奥克兰": "Arrive at Auckland Airport",
  "前往奥克兰市中心酒店": "Travel to the central Auckland hotel",
  "若前一晚住奥马鲁，直接出发前往基督城机场，约245公里 / 3小时6分纯驾驶（2026-07-29 Google 地图实查）": "If staying in Ōamaru, drive directly to Christchurch Airport; about 245 km / 3 hr 6 min driving (checked on Google Maps, 29 Jul 2026)",
  "预留加油、午餐和道路延误缓冲，仍按15:30还车；不再安排基督城市区半日": "Keep buffer for fuel, lunch and road delays, and still return the car at 15:30; skip the Christchurch city half-day",
  "完成入境、取行李": "Clear immigration and collect baggage",
  "步行入住机场酒店": "Walk to Novotel Auckland Airport",
  "前往国内航站楼": "Go to Auckland Airport Domestic Terminal",
  "NZ619 起飞": "NZ619 departs",
  "抵达、机场午餐": "Arrive at Queenstown Airport and have lunch",
  "Budget 取车（已预订）": "Pick up the booked Budget rental car",
  "入住、湖边散步": "Check in at Holiday Inn Queenstown Remarkables Park, then drive to the lakefront",
  "皇后镇花园、湖边": "Queenstown Gardens and the lakefront",
  "市区午餐": "Lunch in central Queenstown",
  "Skyline 缆车": "Skyline Queenstown gondola",
  "咖啡、逛街": "Coffee and shopping",
  晚餐: "Dinner",
  "皇后镇出发": "Leave Queenstown",
  "Bob’s Cove": "Bob's Cove Track",
  "Bennett’s Bluff": "Bennett's Bluff Lookout",
  "格林诺奇午餐": "Lunch in Glenorchy",
  "Lagoon 木栈道": "Glenorchy Lagoon Boardwalk",
  "回到皇后镇": "Return to Queenstown",
  "慢早餐、湖边散步": "Slow breakfast and a lakefront walk",
  "皇后镇花园或自由逛街": "Queenstown Gardens or free time in town",
  "步行到 Steamer Wharf，按预订场次办理登船": "Walk to Steamer Wharf and check in for the booked sailing",
  "瓦卡蒂普湖往返巡游、Walter Peak 烧烤与农场表演": "Lake Wakatipu cruise, Walter Peak barbecue and farm show",
  "返回皇后镇，自由喝咖啡或回酒店": "Return to Queenstown; have coffee or go back to the hotel",
  "简单晚餐": "Light dinner",
  "退房出发": "Check out and depart",
  "箭镇、午餐": "Arrowtown and lunch",
  "前往瓦纳卡": "Drive towards Wānaka",
  "Crown Range 观景": "Stop at Crown Range Summit",
  "Cardrona 咖啡": "Coffee at Cardrona Hotel",
  "抵达瓦纳卡": "Arrive in Wānaka",
  "湖边、That Wanaka Tree": "Lakefront and That Wānaka Tree",
  午餐: "Lunch",
  "Puzzling World 或 Mount Iron": "Puzzling World or Mount Iron Track",
  "咖啡、看日落": "Coffee and sunset",
  "Lindis Pass 观景": "Stop at Lindis Pass Viewpoint",
  "Omarama 午餐、加油": "Lunch and fuel in Omarama",
  "Lake Pukaki 短停": "Short stop at Lake Pukaki",
  "先到库克山机场办理直升机报到": "Check in for the helicopter flight at Mount Cook Airport",
  "Glacier Highlights 直升机，45分钟含约10—15分钟冰川降落": "Glacier Highlights helicopter flight: 45 minutes including a 10–15 minute glacier landing",
  "在库克山用餐、补充保暖衣物": "Have dinner at Aoraki / Mount Cook and add warm layers",
  "Big Sky 前休息；活动后驾车返回 Twizel": "Rest before Big Sky, then drive back to Twizel after the activity",
  "Big Sky 前休息；活动后返回已预订的 Mt Cook Motel": "Rest before Big Sky, then return to the booked Mt Cook Motel",
  "酒店晚餐、补充保暖衣物": "Dinner at the hotel; add warm layers",
  "Big Sky Stargazing 观星，约75—90分钟": "Big Sky Stargazing, approximately 75–90 minutes",
  "仅在前一日停飞时，候补直升机首班": "First backup helicopter flight only if the previous day was cancelled",
  "最晚离开库克山": "Leave Aoraki / Mount Cook no later than this time",
  "抵达蒂卡波，教堂、湖边、午餐": "Arrive at Lake Tekapo: church, lakefront and lunch",
  "前往基督城": "Drive to Christchurch",
  "抵达、入住": "Arrive and check in",
  "野生动物改线：库克山经蒂卡波短停后前往奥马鲁，约281公里 / 3小时26分纯驾驶（2026-07-29 Google 地图实查）": "Wildlife option: drive from Aoraki / Mount Cook to Ōamaru with a short Lake Tekapo stop; about 281 km / 3 hr 26 min driving (checked on Google Maps, 29 Jul 2026)",
  "奥马鲁港边步道找新西兰海狗；保持至少20米，不喂、不摸、不挡住海狗与海之间的路线": "Look for New Zealand fur seals along Ōamaru Harbour; keep at least 20 m away, never feed or touch them, and do not stand between a seal and the sea",
  "提前30分钟抵达 Ōamaru Blue Penguin Colony；晚场禁止拍照、录像及亮屏": "Arrive at Ōamaru Blue Penguin Colony 30 minutes early; cameras, filming and bright screens are prohibited during evening viewing",
  "小蓝企鹅归巢：General 成人 NZD 50、余量5+；Premium 成人 NZD 70、余量5（2026-07-29核验）": "Little penguin evening viewing: General NZD 50 per adult with 5+ places; Premium NZD 70 with 5 places (verified 29 Jul 2026)",
  "市区、Riverside Market": "Central Christchurch and Riverside Market",
  "植物园或咖啡": "Christchurch Botanic Gardens or coffee",
  "New Regent Street 或自由活动": "New Regent Street or free time",
  "基督城晚餐": "Dinner in Christchurch",
  "整理行李并确认 JQ242 航班状态": "Pack and recheck JQ242 flight status",
  "基督城自由活动、午餐": "Free time and lunch in Christchurch",
  "加满油并前往基督城机场": "Refuel and drive to Christchurch Airport",
  "Budget 还车": "Return the car to Budget",
  "JQ242 起飞": "JQ242 departs",
  "市中心退房、前往接驳集合点": "Check out and go to the transfer meeting point",
  "奥克兰出发的霍比屯 + 罗托鲁瓦联程接驳或小团": "Auckland–Hobbiton–Rotorua through-transfer or small-group tour",
  "接驳前往罗托鲁瓦": "Transfer to Rotorua",
  "抵达罗托鲁瓦、入住": "Arrive in Rotorua and check in",
  "罗托鲁瓦前往奥克兰机场的巴士或私人接驳": "Coach or private transfer from Rotorua to Auckland Airport",
  "抵达奥克兰机场、晚餐并整理行李": "Arrive at Auckland Airport, have dinner and organise baggage",
  "从市中心酒店出发": "Leave the central Auckland hotel",
  "Queen Street、Britomart": "Queen Street and Britomart",
  "Commercial Bay 午餐": "Lunch at Commercial Bay",
  "Westfield Newmarket": "Westfield Newmarket",
  "返回市中心酒店": "Return to the central Auckland hotel",
  出发: "Depart",
  "抵达 The Shire’s Rest": "Arrive at The Shire's Rest",
  "霍比屯游览": "Hobbiton Movie Set tour",
  "抵达罗托鲁瓦": "Arrive in Rotorua",
  "可选 Redwoods 轻松散步": "Optional easy walk in Redwoods Whakarewarewa Forest",
  "Te Puia 地热与毛利文化": "Te Puia geothermal and Māori cultural experience",
  "办理国际值机": "Check in for the international flight",
  "前往登机口": "Proceed to the departure gate",
  "MH0132 奥克兰 → 吉隆坡，07:55 抵达 T1": "MH0132 Auckland → Kuala Lumpur; arrive at T1 at 07:55",
  "MH0522 吉隆坡 T1 → 深圳，次日 01:15 抵达": "MH0522 Kuala Lumpur T1 → Shenzhen; arrive at 01:15 the next day",
};

const dayTranslations = {
  "9月28日": ["Shenzhen → Kuala Lumpur → Auckland", "Connect in Malaysia; rest immediately after arrival", "Stay · Novotel Auckland Airport, directly outside the international terminal."],
  "9月29日": ["Auckland → Queenstown", "NZ619 and South Island car pickup", "Stay · Holiday Inn Queenstown Remarkables Park, night 1."],
  "9月30日": ["Queenstown", "Gardens, lakefront and Skyline", "Stay · Holiday Inn Queenstown Remarkables Park; drive and arrange central parking."],
  "10月1日": ["Queenstown ⇄ Glenorchy", "Relaxed half-day lakefront drive", "Stay · Holiday Inn Queenstown Remarkables Park."],
  "10月2日": ["Walter Peak Lake Cruise", "High-country barbecue and farm show", "Stay · Holiday Inn Queenstown Remarkables Park; drive to town and allow for parking."],
  "10月3日": ["Queenstown → Arrowtown → Wānaka", "Crown Range and Cardrona", "Stay · Wanaka Luxury Apartments; use the Cromwell route in bad weather."],
  "10月4日": ["Wānaka", "A full day without changing hotels", "Stay · Wanaka Luxury Apartments; afternoon plans depend on weather."],
  "10月5日": ["Wānaka → Aoraki / Mount Cook", "Glacier helicopter landing and Big Sky Stargazing", "Stay · booked Mt Cook Motel Studio Queen at The Hermitage; breakfast for two is included and check-in is at Mt Cook Lodge."],
  "10月6日": ["Aoraki / Mount Cook → Lake Tekapo → Christchurch", "Backup helicopter flight and the longest South Island drive", "Stay · Novotel Christchurch Cathedral Square."],
  "10月7日": ["Christchurch", "Full city day and South Island driving buffer", "Stay · central Christchurch, night 2."],
  "10月8日": ["Christchurch → Auckland", "Return the car and take JQ242", "Stay · central Auckland for one night after the late arrival."],
  "10月9日": ["Auckland → Hobbiton → Rotorua", "No North Island rental car; transfer to book", "Stay · Millennium Hotel Rotorua; use a taxi or local transfer for Te Puia."],
  "10月10日": ["Rotorua → Auckland Airport", "Te Puia and airport transfer", "No hotel tonight; wait at Auckland Airport after the ground transfer."],
  "10月11日": ["Auckland → Shenzhen", "International return flights via Kuala Lumpur", "Overnight · On board."],
};

const weekdayTranslations = {
  周一: "Monday",
  周二: "Tuesday",
  周三: "Wednesday",
  周四: "Thursday",
  周五: "Friday",
  周六: "Saturday",
  周日: "Sunday",
};

const timeTranslations = {
  上午: "Morning",
  下午: "Afternoon",
  傍晚: "Early evening",
  晚上: "Evening",
  待预订: "To book",
  "计划 12:00": "Planned 12:00",
  时间允许: "If time allows",
  "建议 18:00 前": "Aim before 18:00",
};

function dateInEnglish(date) {
  const match = date.match(/(\d+)月(\d+)日/);
  return match ? `${match[1]}/${match[2]}` : date;
}

function translateDay(day) {
  const [title, subtitle, stay] = dayTranslations[day.date] ?? [day.title, day.subtitle, day.stay];
  return {
    ...day,
    dateKey: day.date,
    displayDate: dateInEnglish(day.date),
    events: day.events.map(([time, text]) => [timeTranslations[time] ?? time, eventTranslations[text] ?? text]),
    stay,
    subtitle,
    title,
    weekday: weekdayTranslations[day.weekday] ?? day.weekday,
  };
}

export const southDaysEn = southDays.map(translateDay);
export const northDaysEn = northDays.map(translateDay);
export const itineraryDaysEn = [...southDaysEn, ...northDaysEn];
